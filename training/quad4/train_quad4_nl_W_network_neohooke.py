"""Voll-Energie-Netz fuer das nichtlineare quad4-Element mit NEO-HOOKE.

Separate Trainingsroutine neben train_quad4_nl_W_network.py (St.-Venant).
Ansatz, Netzform und Verifikation sind identisch -- das Netz lernt die volle
Formaenderungsenergie, Finte und Ke entstehen durch Differentiation:

    W(c,z) = W_net(c,z),   F = grad_z W_net,   K = hess_z W_net

Material: 'NeoHookean1' (in MATLAB auch als 'NeoHooke' ansprechbar),
nu = 0.3, planeStrain; E und d sind exakt herausfaktorisiert.

Was sich gegenueber St.-Venant aendert (siehe quad4_nh_ref.py und
docs/DLFE_quad4_neohooke_plan.md):

  * Referenz: Neo-Hooke-Energie mit ln-J-Barriere (kein Polynom).
  * Zustands-Huelle: Hencky-Dehnung ||ln U|| <= H_MAX und J_MIN <= J <= J_MAX
    statt ||E_green|| <= 0.2 -- deckt etwa +100 % Zug und -45 % Druck ab.
  * Sampler: affiner Anteil als U = exp(H) mit Zug/Druck/Scherung-Mix.
  * Trajektorien aus generate_newton_trajectories_neohooke.m (verzerrte
    Netze, weggesteuerter Zug und Druck).
  * Pflicht-Gates vor dem Training: a, b UND a' (Python-Referenz gegen das
    MATLAB-Element, braucht nh_oracle.mat aus export_nh_oracle.m).
  * Deployment nur bei gruenem Gate c UND erfuelltem Go-Kriterium; sonst
    landet das Netz nur im Sweep-Ordner (QUAD4_DEPLOY_FORCE=1 erzwingt es).

Aufruf:
    python train_quad4_nl_W_network_neohooke.py
    QUAD4_QUICK=1 python train_quad4_nl_W_network_neohooke.py     # Pipeline-Test
    QUAD4_SWEEP=1 python train_quad4_nl_W_network_neohooke.py     # Architektur-Sweep

Deployt nach sourcecode/elements/quad4/quad4_nl_W_network_NeoHookean1.mat.
"""
...
sweep_dir = save_dir / "arch_sweep_nl_W_neohooke"
sweep_dir.mkdir(exist_ok=True)

_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
log_path = sweep_dir / f"quad4_nl_W_nh_{_stamp}.log"

logger = logging.getLogger("quad4_nl_W_nh")
...
# Default h64/d3: die Neo-Hooke-Energie ist kein Polynom und die Huelle ist
# deutlich groesser als bei St.-Venant -- h32 verfehlt schon dort das
# Go-Kriterium.
HIDDEN_DEFAULT = int(os.environ.get("QUAD4_HIDDEN", 64))
DEPTH_DEFAULT = int(os.environ.get("QUAD4_DEPTH", 3))
SWEEP_HIDDEN = [64, 96, 128]
SWEEP_DEPTH = [3, 4]

GO_MEAN = 2.0                 # % mittlerer Total-Fehler (Go-Kriterium)
GO_P99 = 5.0                  # % P99

DEPLOY_FORCE = bool(os.environ.get("QUAD4_DEPLOY_FORCE"))
TRAJ_FILE = "newton_traj_states_neohooke.mat"
...
def _sample_pack(coords_c, z_disp):
    ...
    mE = ref.amplitude(coords_c, z_disp)
    return dict(
...
def build_trajectories(rng):
    ...
    traj_file = save_dir / TRAJ_FILE
    if not traj_file.exists():
        log(f"  WARNUNG: {TRAJ_FILE} fehlt -- Trajektorien-Anteil wird")
        log("           mit synthetischen Samples aufgefuellt. Fuer den vollen")
        log("           Datenmix zuerst generate_newton_trajectories_neohooke.m ausfuehren.")
        return [], []
    ...
    for i in range(N):
        ...
        if not ref.in_state_hull(coords_c, u_c):
            skipped += 1
            continue
...
def dataset_cache_key():
    traj = save_dir / TRAJ_FILE
    traj_stamp = (f"{traj.stat().st_size}_{int(traj.stat().st_mtime)}"
                  if traj.exists() else "none")
    parts = [
        "nh-v1", QUICK, N_GEOM_TRAIN, N_GEOM_VAL, STATES_PER_ELEM,
        FRAC_SYNTH, FRAC_TRAJ, FRAC_AL, traj_stamp,
        ref.H_MAX, ref.J_MIN, ref.J_MAX, ref.NONAFFINE, ref.MODE_P, ref.ROT_SHAPE_DEG,
        ref.ENV_RATIO_MAX, ref.ENV_ANGLE_MIN, ref.ENV_ANGLE_MAX,
...
def load_dataset_cache(key):
    path = save_dir / f"dataset_cache_nh_{key}.npz"
    if not path.exists():
        return None
    try:
        d = np.load(path)
...
def build_mat(model, hidden, depth, va: Batch, res_json):
    ...
    md = {
        ...
        "state_measure": ref.STATE_MEASURE,
        "state_H_max": ref.H_MAX,
        "state_J_min": ref.J_MIN,
        "state_J_max": ref.J_MAX,
        "state_rot_max_deg": 180.0,
        "hidden": hidden,
        "depth": depth,
        "lambda_W": LAMBDA_W, "lambda_F": LAMBDA_F, "lambda_K": LAMBDA_K,
        "train_script": "train_quad4_nl_W_network_neohooke.py",
        "git_hash": git_hash(),
        "seed": 42,
...
def main():
    log("=== Voll-Energie-Netz quad4 nichtlinear, NEO-HOOKE (Sobolev) ===\n")
    log(f"Logdatei: {log_path}")
    log(f"Device:   {device}")
    if device.type == "cuda":
        log(f"GPU:      {torch.cuda.get_device_name(0)}")
    log(f"Material: {ref.MATERIAL}, nu = {ref.NU_TRAIN}, {ref.CONDITION} | E = d = 1")
    log(f"Huelle:   ||ln U|| <= {ref.H_MAX}, J in [{ref.J_MIN}, {ref.J_MAX}]")
    log(f"Modus:    {'QUICK' if QUICK else 'voll'}{' + SWEEP' if SWEEP else ''}\n")

    # --- Gates a und b (ohne Netz, PFLICHT vor jedem Training) --------------
    log("Gates a/b/a' (Referenz, Kette, MATLAB-Oracle -- ohne Netz):")
    ok_ab, _ = ref.run_gates(verbose=True, strict=True, require_oracle=True)
    if not ok_ab:
        raise SystemExit("Gates a/b/a' rot oder nh_oracle.mat fehlt -- Training abgebrochen.")
    ...
    cand_mat = sweep_dir / f"quad4_nl_W_nh_{_stamp}.mat"
    scipy.io.savemat(str(cand_mat), md)
    torch.save(model.state_dict(), sweep_dir / f"quad4_nl_W_nh_{_stamp}.pt")
    with open(sweep_dir / f"quad4_nl_W_nh_{_stamp}_metrics.json", "w") as fh:
        json.dump(res_json, fh, indent=2)
    log(f"\nKandidat gespeichert: {cand_mat}")

    # Deployment nur mit erfuelltem Go-Kriterium (kein stilles Deployen eines
    # zu ungenauen Netzes -- Lektion aus dem h32-St.-Venant-Netz).
    if not (go or DEPLOY_FORCE):
        log("Go-Kriterium verfehlt -> KEIN Deployment. Zum Testen trotzdem "
            "deployen: QUAD4_DEPLOY_FORCE=1 oder den Kandidaten manuell kopieren.")
        raise SystemExit(2)
    out_mat = (element_dir / "quad4_nl_W_network_NeoHookean1.mat").resolve()
    scipy.io.savemat(str(out_mat), md)
    log(f"\nDeployt: {out_mat}" + ("  (ERZWUNGEN, Go verfehlt)" if not go else ""))
